...
class Car(object):
    def __init__(self, x, y, width, height, light, group):
        self.rect = pygame.draw.rect(screen, C1, (x, y, width, height)) #C1 es mi caja que representa un carro
        self.drive = True
        self.light = light
        self.carGroup = group
...
def get_state(time_delay):
    ...
    return closest_top_bottom_car, closest_left_right_car, light_state, time_delay #se devuelve solo 1 estado, ese que corresponde con los min recuerda que el espacio de estados es 10x10x2x4 o sea 800
...
def start():
    ...
    while run:
        ...
        action = q_learning_agent.next_best_action(state=new_state) #P: he activado esta linea, que ha cambiado a cuando la 3a estaba activada
        ...
        #clock.tick(5) #Aqui seteo los FPS
        count = count + 1
        pygame.display.flip()
...
if __name__ == '__main__':
    ...
    q_learning_penalty = [0] #para poder utilizar el truco de python
    PERFORMANCE_STEPS = 1000
    performance_dict = defaultdict(int)

    start() 
    #aqui en python los metodos pueden recibir las var de su main
    #aunque no se pasen de forma explicita por eso start conoce la lista lights
