...
Reward = float
State = Tuple[int, int, int, int] #que es un type hint y como ayuda a entender los in y out de las funciones
Q_Table = Dict[State, List[Reward]] #OJO el rewar de indice 0 es stay el de 1 es switch
...
class QLearningAgent:
    ...
    def _init_q_table() -> Q_Table: 
        ...
        for i, j, k, l in itertools.product(*state_to_record): 
            #itertools.product aqui genero el producto carteciano para tener todos los posibles estados de mi tabla q
            _q_table[(i, j, k, l)] = [0.0, 0.0] # aqui para esa combinacion unica (i, j, k, l) la igualo a [0.0,0.0] el primer elemento de mi lista es el reward si no cambia y el segundo si sis
        return _q_table
    ...
    def calculate_reward(state: State) -> Reward: 
        ...
        """
        luz_semaforo = state[2] #ojo no olvides que corresponde AKA es un reflejo del estado de los sem top y bottom y quue 1 es verde en Y wiii
        # Distancia del carro mas cercano a su semaforo en Y
        car_waiting_in_top_bot_lanes = state[0] == 0 and luz_semaforo == 0  
        car_waiting_in_left_right_lanes = state[1] == 0 and luz_semaforo == 1   # same pero ahora en X
    ...
    def _choose_action(self, state: State) -> Action:
        ...
        if random.uniform(0, 1) < EPSILON:
            ...
            q_entry = self._q_table[state] #me da los rewads de STAY es [0] o SWITCH si es [1] para una clave dada por ejemplo 3,4,1,0
    ...
    def next_best_action(self, state: State): #Esto indica que mi tipo de retorno es Tuple[int, int, int, int]
        """
        ...
        q = self._q_table
        s, a = self._state, self._action.value #P: AQUI recibe el estado y  decision actuales que son considerados los mejores (a)
        s_, a_ = new_state, new_action.value #P: El estado es el mismo que en a (a) pero lo que no entiendo es como new_action podria ser diferente a a si es que ambos reciben el mismo estado como entrada

        # Calculate the differences between q(s',a') - q(s,a)
        #   and then, update the q_table
        q[s][a] += ALPHA * (reward + (GAMMA * q[s_][a_]) - q[s][a]) 
        # ALFA 0.1 y alfa es para ver el impacto de la nueva informacion ya que en verdad es q[s][a] = q[s][a] + AlFA * (reward + (GAMMA * q[s_][a_]) - q[s][a])
        # GAMMA 0.9 dice que tanto primo la ganancia inmediata sobre la futura
